...
# simple integration -- sum of f(z)*dz
# rng: specify range z = [-rng,+rng]
# n: number of data points in [-rng,+rng]

function v = slog_dist_simple ( x, ita, beta, gr, phi, rng, n ) 
  # rng: integrate from -rng to +rng
  # n:   samplings in [-rng, +rng]
  
  z  = (-n:n)/n*rng;
  dz = 1/n*rng;
  ig = slog_dist_integrand( z, x, ita, beta, gr, phi );
  v  = dz .* sum(ig);
endfunction
